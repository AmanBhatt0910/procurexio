...
async function nextReferenceNumber(conn, companyId) {
  ...
  let seq = 1;
  if (rows.length > 0) {
    const lastSeq = parseInt(rows[0].reference_number.split('-')[2], 10);
...
export async function GET(request) {
  ...
  if (search) { conditions.push('(r.title LIKE ? OR r.reference_number LIKE ?)'); params.push(`%${search}%`, `%${search}%`); }
...
export async function POST(request) {
  ...
  if (!companyId || !userId) return Response.json({ error: 'Unauthorized' }, { status: 401 });

  try { requirePermission({ role }, [PERMISSIONS.CREATE_RFQ]); }
  catch (e) { return Response.json({ error: e.message }, { status: 403 }); }
  ...
  try {
    ...
    const [result] = await conn.execute(
      `INSERT INTO rfqs
       ...
       VALUES (?, ?, ?, ?, 'draft', ?, ?, ?, ?)`,
      [companyId, title.trim(), description?.trim() || null, refNumber,
       deadline || null, budget || null, currency || 'USD', userId]
    );
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        await conn.execute(
          `INSERT INTO rfq_items
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [rfqId, companyId, item.description.trim(),
           item.quantity || 1, item.unit || null, item.target_price || null, i]
        );
      }
    }
